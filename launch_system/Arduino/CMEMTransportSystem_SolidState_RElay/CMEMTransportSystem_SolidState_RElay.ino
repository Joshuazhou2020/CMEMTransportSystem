// Change x(on), t(off), y(on), Ver (1,2), then remember to sace: cmd+s
int intervalXOn=10000; //20 is a good number          
int intervalTOff=1;           // interval at which to blink (milliseconds)
int intervalYOn=10000;           // interval at which to blink (milliseconds)
int ver=1;

int test = 0; //test run 

String base="";
String coil="p3_magnet"; //plastic or copper, N means # of intervals
String filename=base+coil+"/"+intervalXOn+"_" + intervalTOff+"_" + intervalYOn+"_v" + ver +".txt";
...
void setup() {
  ...
  Serial.println("                    ");
  Serial.println("#Turn on switch now within 5 seconds...."); // shows pre-scaled value 
  delay(5000);
...
void loop() {
  ...
  if (state =="X_ON" && (currentMillis - previousMillisRelay >= intervalXOn) )
  {
    digitalWrite(ledPin2, HIGH); 
    digitalWrite(ledPin3, HIGH); 
    previousMillisRelay = currentMillis;
  ...
  if (state =="T_OFF" && (currentMillis - previousMillisRelay >= intervalTOff) )
  {
    digitalWrite(ledPin2, HIGH); 
  ...
  if (state =="Y_ON" && (currentMillis - previousMillisRelay >= intervalYOn) )
  {
    digitalWrite(ledPin2, HIGH); 
    digitalWrite(ledPin3, HIGH); 
    previousMillisRelay = currentMillis;
 ...
 // MEASURE CURRENT
  ...
  value = analogRead(analogIn);  
  print ("READ CURRENt SENSOR " + value)


  //Record 5 "OFF" current at the begining
  if (measureState.equals("OFF") && value >= 500 && value <= 530 && index <= 5)
  {
    ts[index] = micros();
    current[index] = value;
    index = index + 1;
  ...
  //Record "ON" current
  if ((value < 500 || value > 530 ) && measureState.equals("OFF"))
  {
    measureState = "ON";
  }
